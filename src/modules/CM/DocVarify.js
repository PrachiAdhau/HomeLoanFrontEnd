...
function DocVarify()
{
  ...
  //  const  {salarySlips,bankCheque} =JSON.parse(userJson)
    ...
    if(!customerID)
     ...
     axios.get(`http://localhost:9095/customer/docVerify/${customerID}`)
     .then(res=>{
         if(res.status===201){
 ...
 const getEditData=_data=>{
     if(customerID)
         {
     axios.get(`http://localhost:9095/customer/getsingleData/${customerID}`)
     .then(res=>{
 ...
 const docVerify = () => {
  ...
  axios.get(`http://localhost:9095/customer/docVerify/${customerID}`)
     .then(res=>{
         if(res.status===201){
             alert("Customer Documents Verified.....")
         }
     })
     .catch(error=>console.log(error));
 ...
 const docRejected=()=>{
  axios.get(`http://localhost:9095/customer/docRejected/${customerID}`)
  .then(res=>{
    if(res.status===201){
        alert("Customer Documents Rejected.....")
        }
    })
      .catch(error=>console.log(error));
 }
          ...
          <div className='card w-50 text-center mt-1 ps-5 pe-5'>
            <form onSubmit={handleSubmit(saveCustomer)}>
              ...
              <div className='mb-4'>
                ...
                <input type='text' className='form-control border border-dark' {...register('customerID', { required: false })}  />
              </div>
              ...
              {/* <img src={'data:image/jpeg;base64,'+customer.allpersonalDocument.photo}  
                               width={100}></img> */}
              ...
              <div className='mb-4'>
                 ...
                 {/* <img src={'data:image/jpeg;base64,'+salarySlips}  
                               width={100}></img> */}
              ...
              <div className='mb-2'>
                <button onClick={docVerify}>Verify</button>
               
              </div>
              <div className='mb-2'>
              <button onClick={docRejected}>Rejected</button>
              </div>
